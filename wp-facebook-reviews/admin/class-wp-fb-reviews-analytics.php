<?php
/**
 * Analytics AJAX handlers for WP FB Reviews.
 *
...
class WP_FB_Reviews_Analytics {
	...
	public function wppro_get_overall_chart_data() {
		...
		// Facebook free plugin: default to Facebook/Twitter when no type filter is set.
		if ( '' === $rtypefilter ) {
			$rtypefilter = "AND (LOWER(type) = 'facebook' OR LOWER(type) = 'twitter')";
		}
	...
	private function build_analytics_where() {
		...
		if ( is_array( $types_array ) && ! empty( $types_array ) ) {
			...
			$where_conditions[] = '(LOWER(type) = %s OR LOWER(type) = %s)';
			$where_values[]     = 'facebook';
			$where_values[]     = 'twitter';
		}
